...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:lmdr="http://spider.sigappfr.org/linkedmdr/#">
		 ...
		 xmlns:rdfs=&quot;http://www.w3.org/2000/01/rdf-schema#&quot;&gt;
			...
			<xsl:for-each select="DCmetadata/*">
				...
				&lt;rdf:Description rdf:about=&quot;&amp;&lmdr;</xsl:text><xsl:value-of select="concat($document,'.',local-name(.),'.',generate-id(.))"/>	
					<xsl:text disable-output-escaping="yes">&quot;&gt;
					&lt;rdf:type rdf:resource=&quot;&amp;&dc;</xsl:text><xsl:value-of select="concat(translate(substring(local-name(.), 1, 1), $lowercase, $uppercase),substring(local-name(.),2,string-length(local-name(.))-1))"/><xsl:text disable-output-escaping="yes">&quot;/&gt;
